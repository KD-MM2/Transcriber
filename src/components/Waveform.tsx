...
// Importing dummy track data
import { TrackContext } from "../context/TrackContext";
import useWindowSize from "../hooks/useWindowSize";
import { tracks } from "./MusicPlayer/dummyData";

// Importing context for track selection
...
const Player = ({
	...
	const isMusicPage = location.pathname.endsWith("/music");
	...
	}, [isShuffleEnabled, audioUrls.length, urlIndex]);
	...
	const onPrev = useCallback(() => {
		setUrlIndex(
			(index) => (index - 1 + audioUrls.length) % audioUrls.length
		);
	}, [audioUrls.length]);
	...
	useEffect(() => {
		if (currentTrackId !== null) {
			const trackIndex = audioUrls.findIndex(
				(track: any) => track.trackId === currentTrackId
			);
			if (trackIndex !== -1) {
	...
	return (
		<div
			className={`flex ${isTablet && !isMinimized ? "flex-col" : "flex-row"} items-center justify-between pl-1 pr-1 text-white`}
		>
			<div className="flex items-center gap-2 pr-2">
				...
				{!isPlaying && isMinimized ? (
					<button
						onClick={onPlayOpen}
						className={`toggle-view-button w-6 h-6 ${isTablet && "mt-4"} rounded-full bg-transparent text-orange-500 flex items-center justify-center text-lg hover:text-white hover:scale-125 transition-transform transition-colors duration-300`}
					>
						<FaPlay />
					</button>
				) : (
					!isMusicPage && (
						<button
							...
							className={`toggle-view-button w-6 h-6 ${isTablet && isMinimized && "mt-4"} rounded-full bg-transparent text-orange-500 flex items-center justify-center text-lg hover:text-white hover:scale-125 transition-transform transition-colors duration-300`}
						>
				...
				{!isMinimized && (
					<>
						<button
							...
							className={`w-6 h-6 rounded-full bg-transparent flex items-center justify-center text-sm transition-transform transition-colors hover:scale-125 duration-300 ${isShuffleEnabled ? "text-white" : "text-orange-500 hover:text-white"}`}
						>
							<FaRandom />
						</button>
						<button
							onClick={onPrev}
							className="w-6 h-6 rounded-full bg-transparent text-orange-500 flex items-center justify-center text-lg hover:text-white hover:scale-125 transition-transform transition-colors duration-300"
						>
							<FaBackward />
						</button>
						<button
							onClick={onPlayPause}
							className="w-8 h-8 rounded-full bg-transparent text-orange-500 flex items-center justify-center text-xl hover:text-white hover:scale-125 transition-transform transition-colors duration-300"
						>
						...
						<button
							onClick={toggleLoop}
							className={`w-6 h-6 rounded-full bg-transparent flex items-center justify-center text-sm transition-transform transition-colors hover:scale-125 duration-300 ${isLoopEnabled ? "text-white" : "text-orange-500 hover:text-white"}`}
						>
							<FaSync />
						</button>
					</>
			...
			{/* Waveform display */}
			<div
				className={`flex-1 audio-file ${isTablet ? "w-full mt-4" : ""}`}
			>
			...
			{!isMinimized && (
				<div className="flex text-right pl-2">
					{!isTablet && (
						<>
							<button
								...
								className="w-8 h-8 pt-3 rounded-full bg-transparent text-orange-500 flex items-center justify-center text-2xl hover:text-white hover:scale-125 transition-transform transition-colors duration-300"
							>
								<FaVolumeUp />
							</button>
							{isVolboxVisible && (
					...
					//Video player and music page navigation
